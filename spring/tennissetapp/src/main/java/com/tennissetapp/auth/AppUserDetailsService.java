...
public class AppUserDetailsService extends JdbcDaoSupport implements UserDetailsService {
    //~ Static fields/initializers =====================================================================================

    public static final String DEF_USERS_BY_USERNAME_QUERY =
            "select username,password, userAccountId, email " +
            "from user_accounts " +
            "where email = ?";
    ...
    public static final String DEF_GROUP_AUTHORITIES_BY_USERNAME_QUERY =
            ...
            "from user_accounts g, group_members gm, group_authorities ga " +
            "where gm.username = ? " +
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        List<AppUserDetails> users = loadUsersByEmail(username);
        ...
        AppUserDetails user = users.get(0); // contains no GrantedAuthority[]

        Set<GrantedAuthority> dbAuthsSet = new HashSet<GrantedAuthority>();

        if (enableAuthorities) {
            dbAuthsSet.addAll(loadUserAuthorities(user.getUserAccountId()));
        }

        if (enableGroups) {
            dbAuthsSet.addAll(loadGroupAuthorities(user.getUserAccountId()));
        }
     ...
     */
    protected List<AppUserDetails> loadUsersByEmail(String email) {
        return getJdbcTemplate().query(usersByUsernameQuery, new String[] {email}, new RowMapper<AppUserDetails>() {
            public AppUserDetails mapRow(ResultSet rs, int rowNum) throws SQLException {
                String username = rs.getString(1);
                String password = rs.getString(2);
                Long userAccountId = rs.getLong(3);
                String email = rs.getString(4);
                return new AppUserDetails(username,password,userAccountId,email,null);
            }
     ...
     */
    protected List<GrantedAuthority> loadUserAuthorities(Long userAccountId) {
        return getJdbcTemplate().query(authoritiesByUsernameQuery, new Long[] {userAccountId}, new RowMapper<GrantedAuthority>() {
            public GrantedAuthority mapRow(ResultSet rs, int rowNum) throws SQLException {
                String roleName = rolePrefix + rs.getString(1); //ROLE_USER
     ...
     * @return a list of GrantedAuthority objects for the user
     */
    protected List<GrantedAuthority> loadGroupAuthorities(Long userAccountId) {
        return getJdbcTemplate().query(groupAuthoritiesByUsernameQuery, new Long[] {userAccountId}, new RowMapper<GrantedAuthority>() {
            public GrantedAuthority mapRow(ResultSet rs, int rowNum) throws SQLException {
     ...
     */
    protected UserDetails createUserDetails(String username, AppUserDetails userFromUserQuery,
...
//                true, true, true, combinedAuthorities);
        ...
        return new AppUserDetails(returnUsername, userFromUserQuery.getPassword(), userFromUserQuery.getUserAccountId(), userFromUserQuery.getEmail(),combinedAuthorities);
    }
